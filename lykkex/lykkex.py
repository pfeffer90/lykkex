...
def get_order_book(asset_pair_id):
    return json.loads(urlopen(
        LykkexConstants.BASE_URL + LykkexConstants.ORDER_BOOKS_RELATIVE_URL + '/' + asset_pair_id).read().decode())


def get_balance(API_KEY):
    return json.loads(urlopen(Request(LykkexConstants.WALLET_URL, headers={'api-key': API_KEY,
...
def send_market_order(API_KEY, asset_pair, asset, order_action='BUY', volume='0.1'):
    ...
    return json.loads(urlopen(Request(LykkexConstants.BASE_URL + 'Orders/market', data,
                                      {'api-key': API_KEY, 'Content-Type': 'application/json'})).read())
...
def control_limit_order(API_KEY, order_id):
    return json.loads(urlopen(Request(LykkexConstants.BASE_URL + 'Orders/' + order_id,
                                      headers={'api-key': API_KEY, 'Content-Type': 'application/json'})).read())
